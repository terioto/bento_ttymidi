#!/bin/bash
# MIDI IN test: capture events on bento_ttymidi:MIDI out.
#
...
# a full round-trip: aplaymidi -> MIDI in -> UART -> MIDI out -> aseqdump.
...
echo "Source port : $BENTO_MIDI_OUT_PORT ($CLIENT:MIDI out)"
...
if [ "$LOOPBACK" = "1" ]; then
    ...
    echo "Loopback mode: playing $MID_FILE while capturing on MIDI out."
    ...
    if grep -qE 'note on|Note on|NoteOn' "$DUMP_FILE" 2>/dev/null; then
        echo ""
        echo "[PASS] Note events received on MIDI out (loopback round-trip)."
        grep -E 'note on|Note on|NoteOn' "$DUMP_FILE" | head -5
        exit 0
    fi

    echo ""
    echo "[FAIL] No note events on MIDI out."
...
echo "Listening on MIDI out for ${CAPTURE_SEC}s ..."
echo ""
...
if grep -qE 'note on|Note on|NoteOn|control change|Control change' "$DUMP_FILE" 2>/dev/null; then
    echo "[PASS] MIDI events received on MIDI out."
